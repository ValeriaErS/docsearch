...
func (i *Indexer) saveDoc(ctx context.Context, doc corpus.Document) error {
	...
	if len(doc.Text) > maxDocSize {
		fmt.Printf("Документ %s слишком большой (%d байт), использую fallback (весь текст одним чанком)\n", doc.Name, len(doc.Text))
	...
	if len(chunks) == 0 || totalChunkLen < len(doc.Text)/3 || len(chunks) < 2 {
		fmt.Printf("Интеллектуальное разбиение дало мусор (%d чанков, %d символов), использую весь текст как один чанк\n", 
...
func (i *Indexer) saveDocByPages(ctx context.Context, doc corpus.Document) error {  //индексирует большой документ постранично
	...
	var batch []map[string]interface{} // сохр все чанки батчами
	const batchSize = 50
...
func (i *Indexer) splitLargePage(text string, docName string, pageNum int) []chunk.IntelligentChunk { // разбивает большую страницу на части без использования splitBySize
