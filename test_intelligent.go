...
func main(){
	text := `# Глава 1
	Тут текст первой главы. Тут про зайку. И состоит из нескольких предложений.
	## Раздел 1.1
	Тут текст раздела. Ещё текст.
	### Пункт 1.1.1
	Тут текст пункта. И ещё.
	# Глава 2
	Тут текст второй главы. Проверяем работу.`
	   
	  chunks:=SplitIntelligent(text,"test.md",30)
	  ...
	  for i:=0;i<len(chunks);i++ {
        ...
        fmt.Println("Токенов:", ch.TokenCount)
        fmt.Println("Текст:", ch.Text)
	  }
}
